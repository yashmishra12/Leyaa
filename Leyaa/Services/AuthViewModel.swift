...
//
//                    guard let doc = snapshot?.documents else {
//                        print("No Doc Found - Room Join Request Update")
//                        return
//                    }
//
...
class AuthViewModel: ObservableObject {
    ...
    func writeUserData(){
        db.collection("users").document(Auth.auth().currentUser?.uid ?? "").updateData(["deviceToken": UserDefaults.standard.string(forKey: "com.yashmisra12.Leyaa.kDeviceToken") ?? "" ]){ error in
            if let error = error {
